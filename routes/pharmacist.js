...
router.get('/invoices', async (req, res) => {
    try {
        ...
        if (search) {
            ...
            invoices = allInvoices.filter(inv => inv.pharmacist_id === pharmacistId);
        }
...
router.get('/invoices/:id', async (req, res) => {
    try {
        ...
        if (req.session.user.role !== 'admin' && invoice.pharmacist_id !== req.session.user.id) {
        ...
        const discount_percentage = sub_total > 0 ? ((discount_amount / sub_total) * 100).toFixed(2) : 0;
...
router.get('/reports', async (req, res) => {
    try {
        ...
        if (date_from && date_to) {
            salesData = await Invoice.getSalesByDateRange(date_from, date_to, pharmacistId);
            totalStats = await Invoice.getSalesStats(pharmacistId, date_from, date_to);
        }
